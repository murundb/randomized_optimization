# Optimization Problem 3: Continuous Peaks
import sys
import time
import numpy as np
...
for n in N:
    ...
    cost_function = mlrose.ContinuousPeaks(t_pct=0.15)
    continuous_peaks_problem = mlrose.DiscreteOpt(length=n, fitness_fn=cost_function, maximize=True, max_val=2)
    ...
    for _ in range(num_trials):
        ...
        rhc_best_state, rhc_best_fitness, rhc_fitness_curve = mlrose.random_hill_climb(continuous_peaks_problem, max_attempts=10, max_iters=np.inf, restarts=5, init_state=None, curve=True)
    ...
    for _ in range(num_trials):
        ...
        # Simulated annealing
        start_time = time.time()
        sa_best_state, sa_best_fitness, sa_fitness_curve = mlrose.simulated_annealing(continuous_peaks_problem, schedule=mlrose.GeomDecay(), max_attempts=10, max_iters=np.inf, init_state=None, curve=True)
    ...
    for _ in range(num_trials):
        ...
        # A Genetic Algorithm
        start_time = time.time()
        ga_best_state, ga_best_fitness, ga_fitness_curve = mlrose.genetic_alg(continuous_peaks_problem, pop_size=200, mutation_prob=0.1, max_attempts=10, max_iters=np.inf, curve=True)
    ...
    for _ in range(num_trials):
        # MIMIC
        start_time = time.time()
        mimic_best_state, mimic_best_fitness, mimic_fitness_curve = mlrose.mimic(continuous_peaks_problem, pop_size=200, keep_pct=0.2, max_attempts=10, max_iters=np.inf, curve=True)
